...
void VEngine::VKShadowPass::addToGraph(FrameGraph::Graph &graph, const Data &data)
{
		...
		[&](VkCommandBuffer cmdBuf, const FrameGraph::ResourceRegistry &registry, const VKRenderPassDescription *renderPassDescription, VkRenderPass renderPass)
	{
		...
		{
			strcpy_s(pipelineDesc.m_shaderStages.m_vertexShaderPath, "Resources/Shaders/shadows_vert.spv");
		...
		for (size_t i = 0; i < data.m_shadowJobCount; ++i)
		{
			...
			for (uint32_t i = 0; i < data.m_subMeshInstanceCount; ++i)
			{
				...
				vkCmdPushConstants(cmdBuf, pipelineData.m_layout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(glm::mat4), sizeof(instance.m_transformIndex), &instance.m_transformIndex);
