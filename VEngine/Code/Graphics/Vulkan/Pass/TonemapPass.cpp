	...
	graph.addPass("Tonemap", QueueType::GRAPHICS, usageCount, passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
		{
			...
			// update descriptor sets
			{
				...
				writer.writeImageInfo(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, registry.getImageInfo(data.m_srcImageHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER), SOURCE_IMAGE_BINDING);
				...
				{
					writer.writeImageInfo(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, registry.getImageInfo(data.m_bloomImageViewHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER), BLOOM_IMAGE_BINDING);
				}
				else
				{
					writer.writeImageInfo(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, registry.getImageInfo(data.m_srcImageHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER), BLOOM_IMAGE_BINDING);
				}
