...
void VEngine::VKGTAOTemporalFilterPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	graph.addPass("GTAO Temporal Filter", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
	{
		...
		// update descriptor sets
		{
			...
			writer.writeImageInfo(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, registry.getImageInfo(data.m_previousImageHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER, data.m_passRecordContext->m_renderResources->m_linearSamplerClamp), PREVIOUS_IMAGE_BINDING);
