...
void VEngine::VoxelizationMarkPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	{
		{ResourceViewHandle(data.m_brickPointerImageHandle), ResourceState::READ_WRITE_STORAGE_IMAGE_FRAGMENT_SHADER},
	};
	...
	graph.addPass("Voxelization Mark", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
		{
			...
			// update descriptor sets
			{
				...
				writer.writeImageInfo(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, registry.getImageInfo(data.m_brickPointerImageHandle, ResourceState::READ_WRITE_STORAGE_IMAGE_FRAGMENT_SHADER), VOXEL_PTR_IMAGE_BINDING);
