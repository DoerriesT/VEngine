...
void VEngine::VKTonemapPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	graph.addPass("Tonemap", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
	{
		...
		// update descriptor sets
		{
			...
			writer.writeImageInfo(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, registry.getImageInfo(data.m_srcImageHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER, data.m_passRecordContext->m_renderResources->m_pointSamplerClamp), SOURCE_IMAGE_BINDING);
		...
		uint32_t luminanceIndex = static_cast<uint32_t>(data.m_passRecordContext->m_commonRenderData->m_currentResourceIndex);
