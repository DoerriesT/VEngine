...
void VEngine::GTAOTemporalFilterPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	graph.addPass("GTAO Temporal Filter", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
	{
		...
		// update descriptor sets
		{
			...
			writer.writeImageInfo(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, registry.getImageInfo(data.m_previousImageHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER), PREVIOUS_IMAGE_BINDING);
			...
			writer.writeImageInfo(VK_DESCRIPTOR_TYPE_SAMPLER, { data.m_passRecordContext->m_renderResources->m_samplers[RendererConsts::SAMPLER_LINEAR_CLAMP_IDX] }, LINEAR_SAMPLER_BINDING);
