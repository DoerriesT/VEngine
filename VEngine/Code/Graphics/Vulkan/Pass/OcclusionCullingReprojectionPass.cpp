...
void VEngine::OcclusionCullingReprojectionPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	graph.addPass("Occlusion Culling Reproject", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
	{
		...
		{
			strcpy_s(pipelineDesc.m_computeShaderStage.m_path, "Resources/Shaders/occlusionCullingReproject_comp.spv");
		...
		// update descriptor sets
		{
			...
			writer.writeImageInfo(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, registry.getImageInfo(data.m_prevDepthImageViewHandle, ResourceState::READ_TEXTURE_COMPUTE_SHADER, data.m_passRecordContext->m_renderResources->m_pointSamplerClamp), DEPTH_IMAGE_BINDING);
