#include "IndirectDiffuseACPass.h"
...
{
	using namespace glm;
#include "../../../../../Application/Resources/Shaders/indirectDiffuseAC_bindings.h"
}

void VEngine::IndirectDiffuseACPass::addToGraph(RenderGraph &graph, const Data &data)
{
	...
	graph.addPass("Indirect Diffuse AC", QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](VkCommandBuffer cmdBuf, const Registry &registry)
		{
			...
			pipelineDesc.setComputeShader(data.m_ssao ? "Resources/Shaders/indirectDiffuseAC_SSAO_ENABLED_comp.spv" : "Resources/Shaders/indirectDiffuseAC_comp.spv", sizeof(specEntries) / sizeof(specEntries[0]), specEntries);
