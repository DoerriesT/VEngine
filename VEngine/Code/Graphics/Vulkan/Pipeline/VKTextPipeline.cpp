#include "VKTextPipeline.h"
#include "Graphics/Vulkan/VKShaderModule.h"
#include "Utility/Utility.h"
#include "Graphics/Vulkan/VKRenderResources.h"

VEngine::VKTextPipeline::VKTextPipeline(VkDevice device, VKRenderResources *renderResources, VkFormat attachmentFormat)
	:m_device(device),
	m_pipeline(),
	m_pipelineLayout()
{
	...
	// create pipeline
	{
		VKShaderModule vertShaderModule("Resources/Shaders/text_vert.spv");
		VKShaderModule fragShaderModule("Resources/Shaders/text_frag.spv");
		...
		VkPipelineColorBlendAttachmentState blendAttachments[] = { colorBlendAttachment };
		...
		pushConstantRange.size = sizeof(float) * 12 + sizeof(uint32_t); // sizeof(glm::vec4) + sizeof(glm::vec4) + sizeof(glm::vec2) * 2 + sizeof(uint32_t)
...
VEngine::VKTextPipeline::~VKTextPipeline()
{
	vkDestroyPipeline(m_device, m_pipeline, nullptr);
	vkDestroyPipelineLayout(m_device, m_pipelineLayout, nullptr);
}

VkPipeline VEngine::VKTextPipeline::getPipeline() const
{
	return m_pipeline;
}

VkPipelineLayout VEngine::VKTextPipeline::getLayout() const
{
