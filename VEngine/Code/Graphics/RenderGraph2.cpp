...
VEngine::rg::Registry2::Registry2(const RenderGraph2 &graph, uint16_t passHandleIndex)
	:m_graph(graph),
	m_passHandleIndex(passHandleIndex)
{
}

Image *VEngine::rg::Registry2::getImage(ImageHandle handle) const
{
	return m_graph.m_imageBuffers[(size_t)handle - 1].image;
}

Image *VEngine::rg::Registry2::getImage(ImageViewHandle handle) const
{
	return m_graph.m_imageBuffers[(size_t)m_graph.m_viewDescriptions[(size_t)handle - 1].m_resourceHandle - 1].image;
}

ImageView *VEngine::rg::Registry2::getImageView(ImageViewHandle handle) const
{
	return m_graph.m_imageBufferViews[(size_t)handle - 1].imageView;
}

VEngine::rg::ImageViewDescription VEngine::rg::Registry2::getImageViewDescription(ImageViewHandle handle) const
{
...
VEngine::rg::ImageDescription VEngine::rg::Registry2::getImageDescription(ImageViewHandle handle) const
{
	const auto &viewDesc = m_graph.m_viewDescriptions[(size_t)handle - 1];
...
Buffer *VEngine::rg::Registry2::getBuffer(BufferHandle handle) const
{
	return m_graph.m_imageBuffers[(size_t)handle - 1].buffer;
}

Buffer *VEngine::rg::Registry2::getBuffer(BufferViewHandle handle) const
{
	return m_graph.m_imageBuffers[(size_t)m_graph.m_viewDescriptions[(size_t)handle - 1].m_resourceHandle - 1].buffer;
}

BufferView *VEngine::rg::Registry2::getBufferView(BufferViewHandle handle) const
{
...
VEngine::rg::BufferDescription VEngine::rg::Registry2::getBufferDescription(BufferViewHandle handle) const
{
	const auto &viewDesc = m_graph.m_viewDescriptions[(size_t)handle - 1];
	const auto &resDesc = m_graph.m_resourceDescriptions[(size_t)viewDesc.m_resourceHandle - 1];
...
bool VEngine::rg::Registry2::firstUse(ResourceHandle handle) const
{
	return m_passHandleIndex == m_graph.m_resourceLifetimes[(size_t)handle - 1].first;
}

bool VEngine::rg::Registry2::lastUse(ResourceHandle handle) const
{
	return m_passHandleIndex == m_graph.m_resourceLifetimes[(size_t)handle - 1].second;
}

void VEngine::rg::Registry2::map(BufferViewHandle handle, void **data) const
{
	const auto &viewDesc = m_graph.m_viewDescriptions[(size_t)handle - 1];
...
void VEngine::rg::Registry2::unmap(BufferViewHandle handle) const
{
	const auto &viewDesc = m_graph.m_viewDescriptions[(size_t)handle - 1];
	const size_t resIdx = (size_t)viewDesc.m_resourceHandle - 1;
	const auto &resDesc = m_graph.m_resourceDescriptions[resIdx];
...
VEngine::rg::RenderGraph2::~RenderGraph2()
{
	reset();
	for (size_t i = 0; i < 3; ++i)
	{
...
void VEngine::rg::RenderGraph2::addPass(const char *name, QueueType queueType, uint32_t passResourceUsageCount, const ResourceUsageDescription *passResourceUsages, const RecordFunc &recordFunc, bool forceExecution)
{
#ifdef _DEBUG
...
VEngine::rg::ImageViewHandle VEngine::rg::RenderGraph2::createImageView(const ImageViewDescription &viewDesc)
{
...
VEngine::rg::BufferViewHandle VEngine::rg::RenderGraph2::createBufferView(const BufferViewDescription &viewDesc)
{
	ResourceViewDescription desc{};
	desc.m_name = viewDesc.m_name;
...
VEngine::rg::ImageHandle VEngine::rg::RenderGraph2::createImage(const ImageDescription &imageDesc)
{
...
VEngine::rg::BufferHandle VEngine::rg::RenderGraph2::createBuffer(const BufferDescription &bufferDesc)
{
	ResourceDescription resDesc{};
...
VEngine::rg::ImageHandle VEngine::rg::RenderGraph2::importImage(Image *image, const char *name, bool clear, const ClearValue &clearValue, ResourceStateData *resourceStateData)
{
...
VEngine::rg::BufferHandle VEngine::rg::RenderGraph2::importBuffer(Buffer *buffer, const char *name, bool clear, const ClearValue &clearValue, ResourceStateData *resourceStateData)
{
...
void VEngine::rg::RenderGraph2::reset()
{
...
void VEngine::rg::RenderGraph2::getTimingInfo(size_t *count, const PassTimingInfo **data) const
{
	*count = m_timingInfoCount;
	*data = m_timingInfos.get();
}

Queue *VEngine::rg::RenderGraph2::getQueue(QueueType queueType) const
{
	return m_queues[static_cast<size_t>(queueType)];
}

void VEngine::rg::RenderGraph2::forEachSubresource(ResourceViewHandle handle, std::function<void(uint32_t)> func)
{
...
void VEngine::rg::RenderGraph2::createPasses(ResourceViewHandle finalResourceHandle, const ResourceStateData &finalResourceState)
{
	...
	if (finalResourceState.m_stateStageMask.m_resourceState != ResourceState::UNDEFINED)
	{
		...
		addPass("Final Resource Transition", static_cast<QueueType>(queueIdx), 1, &resourceUsageDesc, [](CommandList *, const Registry2 &) {}, true);
	}
		...
		for (uint16_t i = 0; i < passCount; ++i)
		{
			if (passRefCounts[i])
			{
				...
				if (!clearResources[i].empty())
				{
					...
					m_passRecordInfo.push_back({ [this, clrResources{std::move(clearResources[i])}] (CommandList *cmdList, const Registry2 &registry)
					{
...
void VEngine::rg::RenderGraph2::createResources()
{
	m_imageBuffers.resize(m_resourceDescriptions.size());
...
void VEngine::rg::RenderGraph2::createSynchronization(ResourceViewHandle finalResourceHandle)
{
...
void VEngine::rg::RenderGraph2::record()
{
	...
	for (const auto &batch : m_batches)
	{
		for (uint16_t i = 0; i < batch.m_passIndexCount; ++i)
		{
			...
			// record commands
			pass.m_recordFunc(cmdList, Registry2(*this, i));
