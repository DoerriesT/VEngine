...
{
	m_transformationComponent.m_rotation = rotation;
	updateViewMatrix();
}
...
void VEngine::Camera::rotate(const glm::vec3 &pitchYawRollOffset)
{
	...
	m_transformationComponent.m_rotation = glm::normalize(tmp * m_transformationComponent.m_rotation * tmp1);
...
void VEngine::Camera::lookAt(const glm::vec3 &targetPosition)
{
	...
	m_transformationComponent.m_rotation = glm::quat_cast(m_cameraComponent.m_viewMatrix);
}
...
{
	return m_transformationComponent.m_rotation;
}
...
void VEngine::Camera::updateViewMatrix()
{
	...
	m_cameraComponent.m_viewMatrix = glm::mat4_cast(m_transformationComponent.m_rotation) * glm::translate(translationMatrix, -m_transformationComponent.m_position);
}
