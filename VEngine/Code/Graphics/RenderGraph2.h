...
namespace VEngine
{
	namespace rg
	{
		...
		{
		public:
			explicit Registry2(const RenderGraph2 &graph, uint16_t passHandleIndex);
		...
		private:
			const RenderGraph2 &m_graph;
			const uint16_t m_passHandleIndex;
		};

		typedef std::function<void(gal::CommandList *cmdList, const Registry2 &registry)> RecordFunc;
