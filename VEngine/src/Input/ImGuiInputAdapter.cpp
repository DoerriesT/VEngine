...
{
	return ((VEngine::UserInput *)userData)->getClipboardText();
}

static void setClipboardText(void *userData, const char *text)
{
	((VEngine::UserInput *)userData)->setClipboardText(text);
}

VEngine::ImGuiInputAdapter::ImGuiInputAdapter(ImGuiContext *context, UserInput &input, Window &window)
	:m_context(context),
	m_input(input),
	m_window(window),
	m_timer(),
...
void VEngine::ImGuiInputAdapter::update()
{
	// make context current
	auto *prevImGuiContext = ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(m_context);
...
void VEngine::ImGuiInputAdapter::resize(uint32_t width, uint32_t height, uint32_t windowWidth, uint32_t windowHeight)
{
	m_width = width;
	m_height = height;
...
void VEngine::ImGuiInputAdapter::onKey(InputKey key, InputAction action)
{
	// make context current
	auto *prevImGuiContext = ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(m_context);
...
void VEngine::ImGuiInputAdapter::onChar(Codepoint charKey)
{
	// make context current
	auto *prevImGuiContext = ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(m_context);
...
void VEngine::ImGuiInputAdapter::onMouseButton(InputMouse mouseButton, InputAction action)
{
	// make context current
	auto *prevImGuiContext = ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(m_context);
...
void VEngine::ImGuiInputAdapter::onMouseMove(double x, double y)
{
}

void VEngine::ImGuiInputAdapter::onMouseScroll(double xOffset, double yOffset)
{
	// make context current
	auto *prevImGuiContext = ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(m_context);
