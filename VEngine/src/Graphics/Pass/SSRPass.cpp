...
void VEngine::SSRPass::addToGraph(rg::RenderGraph &graph, const Data &data)
{
	...
	{
		{rg::ResourceViewHandle(data.m_rayHitPDFImageHandle), {gal::ResourceState::WRITE_RW_IMAGE, PipelineStageFlagBits::COMPUTE_SHADER_BIT}},
		{rg::ResourceViewHandle(data.m_maskImageHandle), {gal::ResourceState::WRITE_RW_IMAGE, PipelineStageFlagBits::COMPUTE_SHADER_BIT}},
	...
	graph.addPass("SSR", rg::QueueType::GRAPHICS, sizeof(passUsages) / sizeof(passUsages[0]), passUsages, [=](CommandList *cmdList, const rg::Registry &registry)
		{
			...
			builder.setComputeShader("Resources/Shaders/hlsl/ssr_cs");
