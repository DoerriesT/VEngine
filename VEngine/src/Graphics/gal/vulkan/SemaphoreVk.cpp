...
VEngine::gal::SemaphoreVk::SemaphoreVk(VkDevice device, uint64_t initialValue)
	:m_device(device),
...
VEngine::gal::SemaphoreVk::~SemaphoreVk()
{
	vkDestroySemaphore(m_device, m_semaphore, nullptr);
}

void *VEngine::gal::SemaphoreVk::getNativeHandle() const
{
	return m_semaphore;
}

uint64_t VEngine::gal::SemaphoreVk::getCompletedValue() const
{
	uint64_t value = 0;
...
void VEngine::gal::SemaphoreVk::wait(uint64_t waitValue) const
{
...
void VEngine::gal::SemaphoreVk::signal(uint64_t signalValue) const
{
