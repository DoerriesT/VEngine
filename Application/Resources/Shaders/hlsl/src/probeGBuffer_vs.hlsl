...
VSOutput main(uint vertexID : SV_VertexID) 
{
	...
	output.worldPos = mul(modelMatrix, float4(position, 1.0));
	output.position = mul(g_Constants.viewProjectionMatrix[g_PushConsts.face], output.worldPos);
	output.texCoord = float2(g_TexCoords[vertexID * 2 + 0], g_TexCoords[vertexID * 2 + 1]);
	output.normal = mul((float3x3)modelMatrix, float3(g_Normals[vertexID * 3 + 0], g_Normals[vertexID * 3 + 1], g_Normals[vertexID * 3 + 2]));
