...
float3 GetTransmittanceToSun(AtmosphereParameters atmosphere, Texture2D transmittance_texture, float r, float mu_s) 
{
...
float4 GetScatteringTextureUvwzFromRMuMuSNu(AtmosphereParameters atmosphere, float r, float mu, float mu_s, float nu, bool ray_r_mu_intersects_ground) 
{
	// Distance to top atmosphere boundary for a horizontal ray at ground level.
	float H = sqrt(atmosphere.top_radius * atmosphere.top_radius - atmosphere.bottom_radius * atmosphere.bottom_radius);
	// Distance to the horizon.
...
void ComputeSingleScatteringTexture(AtmosphereParameters atmosphere,
	Texture2D transmittance_texture, float3 frag_coord,
	out float3 rayleigh, out float3 mie) 
{
	float r;
	float mu;
	...
	bool ray_r_mu_intersects_ground) 
{
	...
	for (int i = 0; i <= SAMPLE_COUNT; ++i) 
	{
		...
		float3 rayleigh_mie_i = GetScattering(atmosphere, scattering_density_texture, r_i, mu_i, mu_s_i, nu, ray_r_mu_intersects_ground)
...
void GetRMuSFromIrradianceTextureUv(AtmosphereParameters atmosphere, float2 uv, out float r, out float mu_s) 
{
...
float3 ComputeDirectIrradianceTexture(AtmosphereParameters atmosphere, Texture2D transmittance_texture, float2 frag_coord) 
{
	float r;
	float mu_s;
...
float3 GetSkyRadianceToPoint(
	...
	Texture3D scattering_texture,
	Texture3D single_mie_scattering_texture,
	float3 camera, float3 position, float shadow_length,
	float3 sun_direction, out float3 transmittance) 
{
	// Compute the distance to the top atmosphere boundary along the view ray,
	// assuming the viewer is in space (or NaN if the view ray does not intersect
	// the atmosphere).
	float3 view_ray = normalize(position - camera);
	float r = length(camera);
	float rmu = dot(camera, view_ray);
	float distance_to_top_atmosphere_boundary = -rmu - sqrt(rmu * rmu - r * r + atmosphere.top_radius * atmosphere.top_radius);
	// If the viewer is in space and the view ray intersects the atmosphere, move
	...
	// Combine the lookup results to get the scattering between camera and position.
...
float3 GetSunAndSkyIrradiance(
	...
	Texture2D transmittance_texture,
	Texture2D irradiance_texture,
	float3 position, float3 normal, float3 sun_direction,
